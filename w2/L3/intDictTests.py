...
def main():
    ...
    # random.seed(1)
    # Uncomment for consistent results
    for i in range(20):
...
def leastb(numBuckets=365, lessThan=0.99):
    high = numBuckets
    low = 0
    ans = int((high + low) / 2)
    while True:
        if collision_prob(numBuckets, ans) < lessThan:
            if collision_prob(numBuckets, ans + 1) >= lessThan:
                return ans
            else:
                low = ans
                ans = int((high + low) / 2)
        elif collision_prob(numBuckets, ans) >= lessThan:
            high = ans
            ans = int((high + low) / 2)
...
if __name__ == '__main__':
    # main()
    print(collision_prob(1000, 50))  # p5_1
    print(collision_prob(1000, 200))  # p5_2
    print(observe_prob(1000, 50, 1000))  # p5_3
    print(observe_prob(1000, 200, 1000))  # p5_4
    print(collision_prob(365, 30))  # p5_5
    print(collision_prob(365, 250))  # p5_5
    print(leastb())
